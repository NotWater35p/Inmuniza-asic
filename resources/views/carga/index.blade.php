        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
            ...
            <div
                ...
                <div class="flex flex-wrap items-center gap-2 w-full md:w-auto">
                    <a href="{{ route('cargas.index', ['fecha_vencimiento_hasta' => now()->format('Y-m-d')]) }}"
            ...
            @if (request()->hasAny([
                ...
                <div class="flex flex-wrap items-center gap-2 px-4 py-2.5 bg-gray-50 border-b border-gray-200 text-xs">
                    <span class="font-medium text-gray-500 flex items-center gap-1"><i data-lucide="filter"
                            class="w-3 h-3"></i>
                    ...
                    @if (request('proximos_vencer'))
                        <span
                            ...
                            <a href="{{ route('cargas.index', request()->except('proximos_vencer')) }}"
                                class="hover:bg-orange-200 rounded-full p-0.5"><i data-lucide="x"
                                    class="w-3 h-3"></i></a>
                        </span>
                    @endif
                ...
                <table class="w-full text-sm text-left text-gray-500">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-4 py-3 font-semibold">Disponible / Inicial</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($cargas as $carga)
                            @php
                                ...
                                if ($diasLeft < 0) {
                                    $badge = ['bg-red-100', 'text-red-700', 'alert-circle', 'Vencida'];
                                } elseif ($diasLeft <= 30) {
                                    $badge = ['bg-orange-100', 'text-orange-700', 'alarm-clock', 'Próx. vencer'];
                                } elseif ($diasLeft <= 90) {
                                    $badge = ['bg-yellow-100', 'text-yellow-700', 'clock', 'Por vencer'];
                                } else {
                                    $badge = ['bg-green-100', 'text-green-700', 'check-circle', 'Vigente'];
                                }
                            @endphp <tr class="hover:bg-gray-50/70 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="font-mono text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded">{{ $carga->lote }}</span>
                                </td>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    ...
                                    @if ($diasLeft >= 0 && $diasLeft <= 90)
                                        <p class="text-xs text-gray-400 mt-0.5 ml-5">{{ $diasLeft }} días</p>
                                    @endif
                                </td>

                                {{-- Celda de cantidad — reemplaza el <td> que muestra solo cantidad --}}
                                <td class="px-4 py-3">
                                    @php
                                        $total = $carga->cantidad;
                                        $disp = $carga->cantidad_disponible ?? $total;
                                        $usado = $total - $disp;
                                        $pct = $total > 0 ? round(($disp / $total) * 100) : 0;
                                        $barColor =
                                        ...
                                        $textColor =
                                            $pct > 50
                                                ...
                                                : ($pct > 20
                                                    ...
                                                    : 'text-red-700');
                                        $bgLight =
                                            $pct > 50 ? 'bg-green-50' : ($pct > 20 ? 'bg-amber-50' : 'bg-red-50');
                                    @endphp
                                    <div class="min-w-[110px]">
                                        {{-- Fracción disponible/total --}}
                                        <div class="flex items-baseline gap-1 mb-1">
                                            <span
                                                class="text-base font-bold {{ $textColor }}">{{ number_format($disp) }}</span>
                                        ...
                                        <div class="flex items-center justify-between mt-1">
                                            <span
                                                class="text-xs text-gray-400">{{ $usado > 0 ? number_format($usado) . ' usados' : 'Sin uso' }}</span>
                                            <span
                                                class="text-xs font-semibold px-1.5 py-0.5 rounded-full {{ $bgLight }} {{ $textColor }}">{{ $pct }}%</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-end">
                                        <button id="acc-btn-{{ $carga->id }}"
                                            data-dropdown-toggle="acc-dd-{{ $carga->id }}"
                            ...
                            @foreach ($cargas->withQueryString()->getUrlRange(1, $cargas->lastPage()) as $page => $url)
                                <li>
                                    @if ($page == $cargas->currentPage())
                                        <span
                                            class="flex items-center justify-center h-8 px-3 text-primary-600 border border-primary-300 bg-primary-50 font-medium">{{ $page }}</span>
                ...
                <div class="border-t border-gray-100 pt-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="date" name="fecha_llegada_desde"
                                value="{{ request('fecha_llegada_desde') }}"
                        ...
                        <div>
                            ...
                            <input type="date" name="fecha_llegada_hasta"
                                value="{{ request('fecha_llegada_hasta') }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2">
                        </div>
                    </div>
                </div>
